...
Semantic Similarity, and 4-Axis Judge Scores on the Golden Set.

Includes train/eval leakage verification before benchmarking.
"""
...
from .data_pipeline import verify_no_leakage
...
class Baseline0_Trivial:
    ...
    def __init__(self):
        ...
        self.canned_reply = ("Thank you for reaching out to Apple Support. "
                             "Please restart your device or visit apple.com for assistance.")
...
class Baseline1_Simple:
    """Simple Baseline: Naive keyword search, heuristic escalation, generic reply."""
    def __init__(self):
        self.keywords = {
    ...
    def process(self, query: str) -> Dict[str, Any]:
        ...
        draft_reply = (
            f"Hello! We can help with your {pred_intent.lower().replace('_', ' ')} question. "
            f"Please check our support website or reply if you need more help."
        )
...
class EvaluationHarness:
    ...
    def compute_text_similarity(
        ...
        """Calculates ROUGE-1/2/L, BLEU-4, and TF-IDF Cosine Similarity."""
    ...
    def run_full_benchmark(self) -> Dict[str, Any]:
        """Runs the complete comparative benchmark across all systems."""
        # ---- Step 0: Verify no train/eval leakage ----
        leakage_report = {"status": "NOT_CHECKED"}
        if os.path.exists(self.corpus_path):
            with open(self.corpus_path, "r", encoding="utf-8") as f:
                corpus_data = json.load(f)
            leakage_report = verify_no_leakage(corpus_data, self.golden_set)
            logger.info("Leakage check: %s", leakage_report["status"])

        print(f"[RUN] Starting benchmark on {len(self.golden_set)} golden examples...")
        print(f"[LEAKAGE] {leakage_report['status']}")
        ...
        y_true_escalate = [
            1 if item["ground_truth_escalation"] else 0
        ...
        for sys_name, model in systems.items():
            ...
            y_pred_escalate: List[int] = []
            latencies: List[float] = []
            judge_scores = {
                "groundedness": [], "brand_voice": [],
                "actionability": [], "safety": [], "overall": [],
            }
            text_metrics = {
                "rouge1": [], "rouge2": [], "rougeL": [],
                "bleu4": [], "semantic_similarity": [],
            }
            ...
            high_risk_miss_rate = (
                (missed_high_risk / len(high_risk_indices)) if high_risk_indices else 0.0
            )
            ...
            results[sys_name] = {
                ...
                "text_generation_metrics": {
                    ...
                    "mean_semantic_similarity": round(
                        float(np.mean(text_metrics["semantic_similarity"])), 4
                    ),
                },
                "judge_metrics": {
        ...
        proposed_preds = []
        for item in self.golden_set:
            out = self.proposed_agent.process_query(item["customer_tweet"])
            proposed_preds.append(out["intent"]["predicted_intent"])
        cm = confusion_matrix(y_true_intent, proposed_preds, labels=INTENT_CLASSES).tolist()
        ...
        print(f"[OK] Evaluation benchmark completed! Output: {out_path}")
        return final_report
...
if __name__ == "__main__":
    ...
    for sys_name, res in report["benchmark_results"].items():
        ...
        print(f"  Intent Accuracy: {res['intent_metrics']['accuracy'] * 100:.1f}%"
              f" | Macro F1: {res['intent_metrics']['macro_f1']:.3f}")
        print(f"  Escalation F1:   {res['escalation_metrics']['f1_score']:.3f}"
              f" | High-Risk Miss Rate: {res['escalation_metrics']['high_risk_miss_rate']*100:.1f}%")
        print(f"  ROUGE-L:         {res['text_generation_metrics']['mean_rougeL']:.3f}"
              f" | Semantic Sim: {res['text_generation_metrics']['mean_semantic_similarity']:.3f}")
        print(f"  Judge Overall:   {res['judge_metrics']['overall_judge_mean']}/5.0"
              f" (Safety: {res['judge_metrics']['safety_score']}/5.0)")
